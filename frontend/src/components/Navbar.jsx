    ...
    <header className="max-w-screen-2xl mx-auto px-6 py-4">
        <nav className="flex justify-between items-center">
            ...
            <div className="relative flex items-center md:space-x-3 space-x-2">
                <div>
                    {currentUser ? (<>
                        <button onClick={(e)=>setIsDropdownOpen((prev)=> !prev)}>
                            <img src={Avatar} alt="user" className={`size-7 rounded-full ${currentUser ? 'ring-2 ring-blue-500' :"" }`}/>
                        </button>
                        {/* show the dropdown */}
                        {isDropdownOpen && (
                ...
                <Link to="/addToCart" className="bg-primary p-1 flex items-center sm:px-6 py-2 gap-3 rounded-sm ">
